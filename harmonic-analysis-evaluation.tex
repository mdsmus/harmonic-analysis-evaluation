...
defficiencies in the less common chord types. Some chord types are
...
instructions to reproduce our analysis, at \url{http://anonymous}.
...
invariance and allows a much simpler network structure.
...
sonority. Both algorithms distinguish enharmonic notes. It should be
noted that, unlike Tsui, our neural networks have an overall worse
accuracy as context is added, as can be seen in section
\ref{sec:discussion}. Our experiments determined that the best amount
of contextual information is the least possible, so \texttt{ec-net}
looks only at one preceding and one following sonority.
...
the best such algorithm. We have also found that a simple memory-based
classifier can outperform some heuristic algorithms. We also presented
a modification to the algorithm described in
...
and Maxwell's expert system \cite{maxwell:expert}. We will also extend
our test corpus incorporating Beethoven sonatas, the Kostka-Payne
