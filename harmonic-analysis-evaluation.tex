...
Evaluating a chord labeling algorithm's performance \note{Evaluating
  the performance of a chord labeling algorithm} with a simple metric
such as the ratio of correct classifications might be useful to give
an idea of how well it generally works, but hides many defficiencies
in the less common chord types \note{``but hides many defficiencies in
  the less common chord types'' é estranho}. Some chord types are
often mistaken for others, as can be seen in table
...
To precisely detect each of these situations \note{what situations?}
we propose a different evaluation methodology. We will evaluate our
algorithms by their precision and recall with respect to each chord
mode, thereby showing where each most easily fails \note{``where each
  most easily fails'' too wordy}.
...
detecting \note{the detection of} each chord type separately. Hence,
every time a diminished chord is misclassified as a non chord tone,
for example, the recall of diminished chords gets smaller, while the
precision of the non-chord tones get smaller. Having this data allows
us to detect biases in the algorithms, and highlight some of their
properties. For example, Pardo and Birmingham's algorithm, as seen in
section \ref{sec:discussion}, has a high recall and a small precision,
while the machine learning algorithms have mostly equivalent precision
and recall.
...
\note{eu ainda acho que vale a pena deixar mais claro que isso e feito
  nos gabaritos, etc} Bach Chorales from the Riemenschneider edition
\cite{bach:371}. The code and data used in this evaluation are
available, together with instructions to reproduce our analysis, at
...
\note{eu acho que esse paragrafo ficou solto, a versao origiral, com
  algumas melhorias estava melhor}

In this paper we evaluate Pardo and Birmingham's algorithm and a few
assorted machine learning techniques. The machine learning algorithms
were trained on the chorales numbered 1, 2, 3, 4, 5, and 6 of the
Riemenschneider corpus, plus some textbook examples of augmented
...
a melodic purpose on the piece) \note{eu tiraria essa frase entre
  parenteses porque nem sempre isso é verdade}. Also, since their
system ignores enharmonic information some fine distinctions are lost,
and a german augmented sixth is shown being classified
\note{erroneously} as a dominant seventh. We have then extended the
algorithm presented in the article by incorporating more chord
templates (giving a total of ten chord templates \note{tira ``chord
...
the features we chose were the four pitch classes of each sonority to
be classified. \note{colocar voz ativa} This, unfortunately, makes it
impossible for this algorithm to generalize to musical styles not very
similar to a four-part chorale, although an extension to allow this is
planned. Our decision tree algorithm is named \texttt{es-tree}. It
distinguishes enharmonic notes but ignores the surrounding context.
\note{esse paragrafo tem muito ``our decision tree''}
...
major and minor. Tsui uses as features \note{eu acho bom definir o que
  é features. ismir é uma conferencia multi-disciplinar} a vector,
each position containing how many times one pitch class sounds in a
given sonority. On training, he \note{the algorithm} first transposes
each sonority to the 12 possible pitch classes to ensure that the
network is invariant under transposition. While interesting, this
...
sonority so that it has C as its lowest note. This codification also
preserves transposition invariance and allows a much simpler network
structure.
...
the best such algorithm \note{``best such algorithm'' is kind of
  ugly}. We have also found that a simple memory-based classifier can
outperform some heuristic algorithms. We also presented a modification
to the algorithm described in \cite{pardo.ea:algorithms} that is
demonstrably better.
